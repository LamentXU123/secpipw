...
class WarningGateTests(unittest.TestCase):
    ...
    def test_ignore_severity_removes_matching_warnings_from_gate(self) -> None:
        stderr = io.StringIO()
    ...
    def test_ignore_severity_keeps_higher_warnings(self) -> None:
        stderr = io.StringIO()
        warnings = [
    ...
    def test_high_warning_blocks_and_instructs_ignore_flag(self) -> None:
        ...
        self.assertEqual(decision.exit_code, 2)
        self.assertIn("high severity warning detected", stderr.getvalue())
        self.assertIn("--spip-ignore-warning", stderr.getvalue())
