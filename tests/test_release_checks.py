...
from secpipw.release_checks import (
...
class ReleaseCheckTests(unittest.TestCase):
    ...
    def test_yanked_release_raises_medium_alert(self) -> None:
        package = FakePackage(
            name="demo",
            version="1.0.0",
            download_url="https://files.pythonhosted.org/packages/demo-1.0.0.whl",
        ...
        alerts = detect_yanked_release_alerts([package])

        self.assertEqual(len(alerts), 1)
        self.assertEqual(alerts[0].severity, Severity.MEDIUM)
        self.assertIn("marked as yanked", alerts[0].message)
        self.assertIn("broken release", alerts[0].message)

    def test_archive_hash_mismatch_uses_available_metadata_without_fetch(self) -> None:
        package = FakePackage(
            name="demo",
            version="1.0.0",
            download_url="https://files.pythonhosted.org/packages/demo-1.0.0.whl",
        ...
        alerts = detect_archive_hash_mismatch_alerts(
            [package],
        ...
        self.assertIn("does not match PyPI metadata digest", alerts[0].message)

    def test_archive_hash_match_does_not_alert(self) -> None:
        package = FakePackage(
            name="demo",
            version="1.0.0",
            download_url="https://files.pythonhosted.org/packages/demo-1.0.0.whl",
            artifact_name="demo-1.0.0.whl",
            archive_hash="sha256=goodcafe",
            metadata={
                "urls": [
                    {
                        "filename": "demo-1.0.0.whl",
